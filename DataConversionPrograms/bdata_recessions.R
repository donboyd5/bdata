...
# create data frame to be used for recession bands in graphs, etc.

# get latest from NBER
ujrec <- "http://data.nber.org/data/cycles/business_cycle_dates.json"
recessions <- read_json(ujrec, simplifyVector = TRUE) %>%
  mutate(across(c(peak, trough), as.Date))
